...
with gr.Blocks(theme=theme, title="NeoRefacer - AI Refacer") as demo:
    ...
    with gr.Tab("Bulk Mode"):
        ...
        with gr.Row():
            bulk_output = gr.Textbox(label="Output", lines=10, interactive=False)
        ...
        def run_bulk(files, dest_face_img, origin_face_img, threshold_val, partial_reface_ratio, progress=gr.Progress()):
            ...
            faces_config = [{
                ...
                'destination': dest_face_img,
                'threshold': threshold_val
            }]

            refacer.prepare_faces(faces_config, disable_similarity=(origin_face_img is None))
    ...
    with gr.Tab("Bulk Video Mode"):
        ...
        with gr.Row():
            bulk_video_output = gr.Textbox(label="Output", lines=10, interactive=False)
            bulk_video_status = gr.Textbox(label="Status", lines=1, interactive=False)
        with gr.Row():
